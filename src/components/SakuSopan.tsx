...
const TARGETS = [
  { value: "atasan", label: "🏢 Atasan / Bos Kerja" },
  { value: "dosen", label: "🎓 Dosen / Guru Sekolah" },
  { value: "pak-rt", label: "🏡 Pak RT / Ibu RW / Tetangga" },
  { value: "rekan", label: "🤝 Rekan Kerja / Mitra" },
  { value: "pelanggan", label: "🛍️ Pelanggan / Pembeli" },
  { value: "keluarga", label: "👵 Keluarga Besar / Orang Tua" },
  { value: "teman", label: "💬 Teman Dekat / Sebaya" },
];

const PURPOSES = [
  { value: "izin-sakit", label: "🤒 Minta Izin Sakit atau Cuti Kerja/Kuliah" },
  { value: "rsvp", label: "💌 Balasan Kehadiran Acara / RSVP Undangan" },
  { value: "komplain", label: "📦 Komplain Paket Rusak / Layanan Kurang Baik" },
  { value: "kirim-tugas", label: "📝 Kirim Tugas Kuliah atau Laporan Kerjaan" },
  { value: "tanya-harga", label: "💰 Bertanya Harga barang atau Ketersediaan Stok" },
  { value: "minta-maaf", label: "⏰ Minta Maaf karena Terlambat atau Batal Janji" },
  { value: "salam", label: "🌟 Selamat Hari Raya atau Silaturahmi Sopan" },
];
...
export default function SakuSopan({ onSpeak, isSpeaking, lang }: SakuSopanProps) {
  ...
  const generateMessages = async () => {
    ...
    const activePurpose = PURPOSES.find((p) => p.value === purpose)?.label || purpose;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Gagal menyusun pesan");
      }
